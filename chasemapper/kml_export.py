...
and the log is the only place the *original* first recorded prediction survives
...
def _triple(value):
    ...
    if lat is None or lon is None or alt is None or not -90 <= lat <= 90 or not -180 <= lon <= 180:
        return None
    return (lat, lon, alt)
...
def collect_flight_data(log_entries, callsigns=None):
    ...
    This may be after launch or after a server restart.
    ...
    for entry in log_entries:
        ...
        if _type == LOG_TELEMETRY:
            point = _triple([entry.get("lat"), entry.get("lon"), entry.get("alt", 0)])
            if point is None:
                continue
            _slot(_call)["track"].append(
                (entry.get("time") or entry.get("log_time") or "", *point)
            )
...
def _flight_folder(callsign, data):
    """Build the per-payload folder: actual path, then first recorded prediction."""
    body = []
    ...
    if prediction:
        ...
        pred = _track_placemark(
            ...
            "predPath",
            prediction["path"],
            "First recorded prediction, logged at %s from %.0f m."
        ...
        body += _folder("First recorded prediction", pred)
...
def _geofence_folder(geofence, profile_name=""):
    ...
    points = [_triple([*p, 0]) if isinstance(p, (list, tuple)) and len(p) == 2 else None for p in polygon]
    ...
    name = "Current geofence - %s" % profile_name if profile_name else "Current geofence"
...
def _overlay_content(path, prefix="overlay_0_"):
    ...
    # KML IDs are document-wide, including styles inside nested folders.
    ids = {}
    for i, node in enumerate(root.iter()):
        original = node.get("id")
        if original:
            replacement = prefix + str(i)
            ids.setdefault(original, replacement)
            node.set("id", replacement)
    for node in root.iter():
        if node.tag.split("}")[-1] == "styleUrl" and node.text and node.text.startswith("#"):
            node.text = "#" + ids.get(node.text[1:], prefix + node.text[1:])
        if node.get("targetId") in ids:
            node.set("targetId", ids[node.get("targetId")])
...
def _overlay_folders(overlays):
    ...
    for index, overlay in enumerate(overlays or []):
        ...
        if not path or not os.path.isfile(path):
            ...
            continue
        content = _overlay_content(path, "overlay_%d_" % index)
        if not content:
            continue
        folders += _folder(
            "Current overlay - " + (overlay.get("name") or os.path.basename(path)),
            content,
...
def build_flight_kml(
    ...
    description = "\n".join(
        [
            ...
            "Context layers, when included, use CURRENT configuration at export time; historical configuration was not recorded.",
