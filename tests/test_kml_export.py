...
class BuildFlightKmlTests(unittest.TestCase):
    ...
    def test_output_is_well_formed_and_has_every_layer(self):
        ...
        self.assertIn("First recorded packet", marks)
...
class ExportValidationTests(unittest.TestCase):
    def test_invalid_coordinates_are_omitted(self):
        for lat, lon, alt in [(float('nan'), 0, 1), (91, 0, 1), (0, 181, 1), (0, 0, float('inf'))]:
            self.assertEqual(kml_export.collect_flight_data([telemetry('BAD', lat, lon, alt)]), {})
            self.assertIsNone(kml_export._triple([lat, lon, alt]))

    def test_overlay_ids_and_references_are_unique(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, 'style.kml')
            with open(path, 'w') as f:
                f.write('<kml><Document><Style id="actualPath"/><Placemark><styleUrl>#actualPath</styleUrl></Placemark></Document></kml>')
            xml, _ = kml_export.build_flight_kml(ASCENT, overlays=[{'path': path}, {'path': path}])
        root = parse(xml)
        ids = [n.get('id') for n in root.iter() if n.get('id')]
        self.assertEqual(len(ids), len(set(ids)))
        for node in root.findall('.//kml:styleUrl', KML_NS):
            self.assertIn(node.text[1:], ids)
        self.assertIn('CURRENT configuration at export time', xml)
