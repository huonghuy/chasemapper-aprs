...
function loadKmlOverlayData(layer){
    ...
    if (layer._kml_loading || layer._kml_loaded) return;
    layer._kml_loading = true;
    function cleanup() {
        layer._kml_loading = false;
        layer.off("ready", ready);
        layer.off("error", failed);
    }
    function ready() { cleanup(); layer._kml_loaded = true; }
    function failed(e) {
        cleanup();
        layer._kml_loaded = false;
        layer.clearLayers();
        console.log("Error loading KML overlay; toggle to retry", e);
    }
    layer.on("ready", ready).on("error", failed);
    try { omnivore.kml(layer._kml_url, null, layer); }
    catch (e) { failed(e); }
}
...
function loadConfiguredKmlOverlays(config, map){
    ...
    // Context overlays sit below recovery layers.
